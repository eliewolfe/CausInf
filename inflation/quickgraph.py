...
if __name__ == '__main__':
    ...
    names, parents_dict, roots_dict, determinism_checks, filtered_determinism_checks = LearnParametersFromGraph(TGraph)
    print(names)
    ...
    for screenoffs in filtered_determinism_checks:
        ...
        """
        Elie's version would look like this:
        U2 = set([])
        U2.update(verts["roots_of"][X] for X in Xs)
        U2.remove(U1)
        
        Z = set(nonroot_vertices)
        Z.remove(Y)
        Z.difference_update(X)
        for u2 in U2:
            Z.difference_update(verts["descendants"][u2])
